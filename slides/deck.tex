...
\begin{frame}{Results (real leaderboard)}
  ...
  \begin{tabular}{lc}
    ...
    \midrule
    Leak (\emph{declined}) & 1.000 \\
    \textbf{\color{ok}Honest, full (with assignment)} & \textbf{\color{ok}0.930} \\
    Honest, deployable (single-query) & 0.725 \\
    \midrule
  ...
  \vfill
  \footnotesize
  \textbf{Honest caveat (we say it first):} the $0.930\!-\!0.725=\!+0.205$ gap is the Hungarian
  \emph{assignment} exploiting the eval bijection — a disclosed \emph{batch} optimization, not
  deployable single-query anatomy. The deployable number is \textbf{0.725}.
\end{frame}
